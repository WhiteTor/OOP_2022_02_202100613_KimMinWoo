	...
	public static void printInfo() {
		while(true) {
			...
			if(subject_aspect.equals("돌아가기")) {
				...
				aspect num1 = new music();   // 주석 5: 다형성을 이용한 변수 생성
				num1.printInfo_aspect();
				num1.end();
				}
			else if (subject_aspect.equals("2")) {
				aspect num2 = new drama();   // 주석 5: 다형성을 이용한 변수 생성
				num2.printInfo_aspect();
				num2.end();
				}
			else if (subject_aspect.equals("3")) {
				aspect num3 = new food();   // 주석 5: 다형성을 이용한 변수 생성
				num3.printInfo_aspect();
				num3.end();
				}
			else if (subject_aspect.equals("4")) {
				aspect num4 = new webtoon();   // 주석 5: 다형성을 이용한 변수 생성
...
class aspect extends ask_continue{   // 주석 5: 다형성을 위해 생성한 부모 클래스
...
class music extends aspect {
	@Override   // 주석 5: 부모클래스로부터 상속받은 함수를 재정의
	public void printInfo_aspect() {
